var baseURL = 'https://127.0.0.1:8443/onw/'
...
$(document).ready(function () {
    ...
    $('#create_room_button').click(function () {
        $.ajax({
            url: baseURL + $('#player_name_input').val() + '/create',
            type: 'POST',
    ...
    $('#config_card_div > :checkbox').each(function(idx, element){
        $(element).bind('click', {}, function(e){
            ...
            $.ajax({
                url: baseURL + $('#player_name_input').val() + '/cards/' + roomID,
                type: 'POST',
    ...
    $('#ready_checkbox').click(function () {
        ...
        //清空消息栏
        $('#messages_div').html('')
        ajaxGet(baseURL + $('#player_name_input').val() + '/ready/' + $('#room_id_span').html() + '/' + $('#ready_checkbox').prop('checked'))
    })

    $('#exit_button').click(function(){
        var roomID = $('#room_id_number').val()
        ajaxGet(baseURL + $('#player_name_input').val() + '/leave/' + roomID)
        closeTopic()
    ...
    $('#deck_div > button').click(function(){
        var url = baseURL + $('#player_name_input').val() + '/card/' + $('#room_id_number').val() + '/' + this.value
        ajaxGet(url)
    })
    ...
    $('#seat_div > .seat').click(function(e){
        ...
        var url = baseURL + $('#player_name_input').val() + '/seat/' + roomID + '/' + $(this).attr('location')
        ajaxGet(url)
    })
    ...
    function subscribeRoom(joinRoomAfterSubscribe) {
        ...
        topicStomp.connect('guest', 'guest', function (frame) {
            ...
            if(joinRoomAfterSubscribe){
                ajaxGet(baseURL + $('#player_name_input').val() + '/join/' + roomID)
            }
        })
    }
})
